...
<button class="toc-toggle" id="tocToggle" aria-label="投稿一覧を開く">
  <span class="icon">☰</span>
...
async function initialize() {
  ...
  // 1. 全カードをメモリに収集（公開記事）
  const allCards = PUBLIC_POSTS.map(p => ({
    ...
    if (categories.length > 0) {
      ...
      filterWrap.addEventListener('click', e => {
        ...
        tocList.querySelectorAll('a[data-cat]').forEach(a => {
          const match = !selected || a.dataset.cat === selected;
...
// =====================================================
// サイドバー開閉（script.js は #article 前提のため index 用にインライン）
// =====================================================
(function() {
